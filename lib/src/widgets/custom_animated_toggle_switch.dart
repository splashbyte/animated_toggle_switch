...
Widget _defaultIndicatorAppearingBuilder(BuildContext context, double value,
    Widget indicator) {
  ...
  Widget vertical() {
    ...
    return RotatedBox(
        ...
        child: CustomAnimatedToggleSwitch<T>._(
          ...
          current: current,
          values: values,
          iconBuilder: (context, global, local) =>
              RotatedBox(
                quarterTurns: -1,
...
class _CustomAnimatedToggleSwitchState<T>
  ...
  double _doubleFromPosition(double x,
  ...
  /// It is mainly intended as a helper function for the build method.
  TogglePosition<T> _togglePositionFromRealPosition(double x,
      DetailedGlobalToggleProperties<T> properties) {
            ...
            builder: (context, loadingValue, child) =>
                AnimatedBuilder(
                    ...
                    builder: (context, child) {
                      ...
                      GlobalToggleProperties<
                          T> properties = GlobalToggleProperties(
                      ...
                      Widget child = Padding(
                        ...
                        child: LayoutBuilder(
                          builder: (context, constraints) {
                            ...
                            double position =
                                (indicatorSize.width + spacing) *
                                    positionValue +
                                    indicatorSize.width / 2;

                            double leftPosition = textDirection ==
                                TextDirection.rtl
                                ? width - position
                                : position;
                            ...
                            List<Widget> stack = <Widget>[
                              ...
                              if (widget.iconArrangement ==
                                  IconArrangement.overlap)
  ...
  List<Widget> _buildBackgroundStack(BuildContext context,
      DetailedGlobalToggleProperties<T> properties) {
    return [
  ...
  List<Widget> _buildBackgroundRow(BuildContext context,
      DetailedGlobalToggleProperties<T> properties) {
...
class _AnimationInfo {
  ...
  const _AnimationInfo._internal(this.start,
      this.end, {
        this.toggleMode = ToggleMode.none,
        this.loading = false,
      });

  const _AnimationInfo.animating(this.start,
      this.end, {
        this.loading = false,
